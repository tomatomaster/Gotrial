...
func equal(x, y reflect.Value, seen map[comparison]bool) bool {
	...
	if x.Type() != y.Type() {
		log.Printf("Type:%v Type:%v", x.Type(), y.Type())
		return false
	}
	...
	case reflect.Float32, reflect.Float64:
		sub := x.Float() - y.Float()
		if nano > math.Abs(sub) {
			log.Printf("%e ≒ %e", x.Float(), y.Float())
			return true
		}
		return x.Float() == y.Float()

	case reflect.Complex64, reflect.Complex128:
		sub := x.Complex() - y.Complex()
		if nano > cmplx.Abs(sub) {
			log.Printf("%e ≒ %e", x.Complex(), y.Complex())
			return true
		}
