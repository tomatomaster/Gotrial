...
func (s *IntSet) Has(x int) bool {
	word, bit := x/BITSIZE, uint(x%BITSIZE)                   //word = bisSize
	return word < len(s.words) && s.words[word]&(1<<bit) != 0 //words[2] ならば words[1] words[0]は11111111...1111
}

func (s *IntSet) Add(x int) {
	word, bit := x/BITSIZE, uint(x%BITSIZE) //128 word(2) bit(0) [1 0 0]
...
func (s *IntSet) Copy() *IntSet {
	c := make([]uint, len(s.words), cap(s.words))
	copy(c, s.words)
